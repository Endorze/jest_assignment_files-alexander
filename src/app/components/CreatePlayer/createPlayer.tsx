...
const CreatePlayer = ({ setPlayer }: { setPlayer: (s: string) => void }): ReactElement => {
    ...
    return (
        <div data-testid='createPlayer' className="flex flex-col gap-4 bg-[var(--color-bg-main)] text-white px-24 py-12  shadow-2xl shadow-gray-500">
            ...
            <label htmlFor='name' className="tracking-wide text-2xl mb-3 underline">Pick a name</label>
            
            <input id='name' className="bg-white text-gray-900 px-2 placeholder:text-gray-500 outline-none" type="text" placeholder="Enter name..." value={currentInput} onChange={(e) => setCurrentInput(e.target.value)}
                maxLength={15}></input>
